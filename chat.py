...
Interactive Streamlit chat application that combines RAG retrieval,
LLM generation, and real-time RAGAS evaluation for NASA mission Q&A.
"""
...
def display_evaluation_metrics(scores: Dict[str, float]):
    ...
    for metric_name, score in scores.items():
        if isinstance(score, (int, float)):
            ...
            st.sidebar.progress(min(max(score, 0.0), 1.0))
...
def main():
    ...
    with st.sidebar:
        ...
        mission_filter = st.selectbox(
            ...
            help="Restrict retrieval to a specific mission",
        )
    ...
    if prompt := st.chat_input("Ask about NASA space missions..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching documents and generating response..."):
                # Retrieve relevant documents with optional mission filter
